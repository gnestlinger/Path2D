classdef Frenet2CartTestSpline < matlab.unittest.TestCase
    ...
    methods (Test)
        function testFrenet2Cart(testCase, DoPlot)
            obj = SplinePath.pp2Path(mkpp([0 1 2 3], [1 0; 0 0; 1 1; 0 0; 1 2; 0 0], 2));
            [xy,Q,idx,tau] = obj.frenet2cart([0 1; 1 -1; 2.25 2; 3 -2], DoPlot);
        ...
        function testOutOfBound(testCase, DoPlot)
            obj = SplinePath(0:2, cat(3, [1 1;0 0], [0 1;1 1]));
            
            % Pre-path/post-path  solution
            [xy,Q,idx,tau] = obj.frenet2cart([-1 1; 3 -1], DoPlot);
        ...
        function testCircuit(testCase, DoPlot)
            ...
            s = obj.length();
            [xy1,Q1,idx1,tau1] = obj.frenet2cart([0 1; 0 -1], DoPlot);
            [xy2,Q2,idx2,tau2] = obj.frenet2cart([s 1; s -1], DoPlot);
