classdef PointProjectionTestSpline < matlab.unittest.TestCase
    ...
    methods (Test)
        function testUniqueSolutions(testCase, DoPlot)
            ...
            [Q,idx,tau,dphi] = pointProjection(obj, [3 0], [], DoPlot);
        ...
        function testMultipleSolutions(testCase, DoPlot)
            ...
            [Q,idx,tau,dphi] = pointProjection(obj, [0 6], [], DoPlot); % [6 3]
            sq11 = sqrt(11/2);
        ...
        function testNoSolution(testCase, DoPlot)
            ...
            [Q,idx,tau,dphi] = pointProjection(obj, [4 1], [], DoPlot);
        ...
        function testSolutionAtBreak(testCase, DoPlot)
            sp = SplinePath([0 1 2 3], cat(3, [1 1 1;0 0 0], [0 1 2; 1 1 1]));
            [Q,idx,tau,dphi] = sp.pointProjection([1 2], [], DoPlot);
